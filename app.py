...
def generate_plots(N, mu, sigma2, S):
    ...
    plt.plot(X, slope * X + intercept, color="red", label=f"y = {slope:.2f}x + {intercept:.2f}")  # Regression line
    plt.xlabel("X")  # Label x-axis
    plt.ylabel("Y")  # Label y-axis
    plt.title(f"Regression Line: y = {slope:.2f}x + {intercept:.2f}")  # Title with regression line equation
    plt.legend()
    plt.savefig("static/plot1.png")  # Save plot to "static/plot1.png"
    plt.close()
    ...
    bins = 'auto'  # Adjusted to 'auto' for smoother histograms with higher S
    plt.figure(figsize=(10, 5))

    # Plot histogram of slopes and intercepts
    plt.hist(slopes, bins=bins, alpha=0.5, color="blue", label="Slopes")
    plt.hist(intercepts, bins=bins, alpha=0.5, color="orange", label="Intercepts")
    
    # Add vertical lines for initial slope and intercept
